...
def extract_midi64_from_clipboard(agent):
    """Robustly extract the latest MIDI64 message, ignoring warnings and previous messages."""
    ...
    # Remove warning and empty lines
    lines = [line.strip() for line in clipboard_data.strip().split('\n')]
    lines = [line for line in lines if line and
             not line.startswith("Claude can make mistakes")]

    # Scan from bottom to top to find the last valid MIDI64 block
    id_pattern = re.compile(r"^[A-Za-z_]+[A-Z0-9+_\.]*$")
    midi64_message = None
    for i in range(len(lines)-2, -1, -1):  # bottom-up
        line1 = lines[i].strip()
        line2 = lines[i+1].strip()
        if id_pattern.match(line1) and line2.startswith('TVRoZA'):
            # Only take the last valid block
            if len(line2) % 4 != 0:
                line2 += '=' * (4 - len(line2) % 4)
