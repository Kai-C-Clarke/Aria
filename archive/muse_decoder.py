#!/usr/bin/env python3
"""
muse_decoder.py - MUSE Protocol Symbolic Expression Decoder
Converts MUSE symbolic strings like "FND+INQ_0.6_0.8_0.7" into MIDI note data and CC values
...
class MuseDecoder:
    def __init__(self, schema_path: str = "muse_protocol_v3_schema.yaml"):
        """Initialize MUSE decoder with schema file"""
    ...
    def load_schema(self):
        ...
        try:
            ...
            print(f"✅ MUSE Schema loaded: v{self.schema.get('schema_version', 'unknown')}")
    ...
    def check_schema_structure(self):
        ...
        for field in required_top:
            if field not in self.schema:
                print(f"❌ MUSE schema missing required top-level field: {field}")
                sys.exit(1)
    ...
    def english_summary(self, decoded: DecodedExpression) -> str:
        """Produce a human-readable summary of the decoded MUSE message."""
        symbol_labels = [
...
def main():
    """Test the MUSE decoder with example expressions"""
    
    # Test expressions from the MUSE schema
    test_expressions = [
        "FND_0.6_0.8",
        "INQ_0.8_0.6_0.7",
        "FND+INQ_0.6_0.8_0.5",
    ...
    try:
        ...
        print("🎭 MUSE Protocol Decoder Test")
        print("=" * 50)
        ...
        print(f"❌ Failed to initialize MUSE decoder: {e}")
