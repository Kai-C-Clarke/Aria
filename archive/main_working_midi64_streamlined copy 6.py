...
def extract_midi64_from_clipboard(agent):
    """Accept only K2C_##### or C2K_##### blocks, ignore recycled/echoed messages. Always synthesize audio after writing YAML."""
    ...
    logging.info(f"🎵 Accepted MIDI64: {lines[i-1]}")
    ...
    if 'MUSE_' in id_line:
        try:
            muse_expr = id_line.split('MUSE_')[1].split('_A')[0]
            ...
            # If MUSE decode fails, fallback below
    ...
    # ✅ ALWAYS synthesize after writing YAML, with logging!
    logging.info(f"🔊 Synthesizing from YAML: {yaml_path}")
    try:
        synthesize_from_yaml(yaml_path)
        logging.info(f"🔊 Synth finished for {yaml_path}")
    except Exception as e:
        logging.error(f"❌ Synth failed for {yaml_path}: {e}")
    return midi_block
    ...
    for i in range(max_rounds):
        ...
        try:
            if current_message is None:
                # Inject initial symbolic system prompt
                prompt_message = "Prmpt\nTVRoZAAAAAYAAQABAeBNVHJrAAAAMAD/AwMHoQAA/y8A"
                logging.info("📥 Injecting symbolic system prompt: Prmpt")
    
                kai_smart_desktop_switch("Kai")
                time.sleep(1)
                kai_safe_click("Kai")
                kai_text_injection(prompt_message, "Kai")
                time.sleep(4)

                # Wait for Kai's first musical reply (K2C_00001)
                kai_smart_desktop_switch("Kai")
                time.sleep(3)
                kai_safe_click("Kai")
                time.sleep(1)
                logging.info("🎯 Extracting Kai's first response to Prmpt...")
    
                copy_success = simple_drag_copy("Kai")
                if not copy_success:
                    logging.error("❌ Copy failed from Kai")
                    return

                current_message = extract_midi64_from_clipboard("Kai")
            if not current_message:
                logging.warning("⚠️ No valid response from Kai after Prmpt")
                return

            logging.info("✅ Kai's first response extracted")
            message_path = write_midi64_message(current_message, "Kai")
            message_files.append(message_path)
            continue  # Proceed to first Claude reply
            ...
            if not response:
                ...
                current_message = generate_midi64_message(from_agent=listener, to_agent=speaker, session=session)
                continue
