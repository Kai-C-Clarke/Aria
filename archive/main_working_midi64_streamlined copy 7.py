...
def extract_midi64_from_clipboard(agent):
    ...
    if ENABLE_MUSE_PROTOCOL:
        try:
            # Try to extract MUSE expression from the MIDI data or use fallback
            # For now, we'll generate a basic consciousness YAML since the new format doesn't embed MUSE in ID
            logging.info("🧠 Attempting MUSE-based consciousness generation")
