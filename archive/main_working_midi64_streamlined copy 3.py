...
def simple_drag_copy(agent):
    ...
    # Use exactly the same logic for both agents, just change coordinates:
    if agent == "Kai":
        start_x, start_y = 188, 214
        end_x, end_y = 857, 835
    elif agent == "Claude":
        # Updated coordinates for Claude per user correction
        start_x, start_y = 1252, 202
        end_x, end_y = 2015, 912

    pyperclip.copy("")
    time.sleep(0.5)
    # Focus by clicking at end position first (as with Kai)
