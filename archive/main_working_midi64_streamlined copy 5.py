...
def extract_midi64_from_clipboard(agent):
    """Extract only the latest Kai_##### or Claude_##### MIDI64 block, ignoring UI artifacts and warnings."""
    ...
    # Remove UI junk and empty lines
    lines = [line.strip() for line in clipboard_data.strip().split('\n') if line.strip()]
    junk_lines = [
        "Copy", "Edit", "Retry",
        "Claude can make mistakes. Please double-check responses."
    ]
    lines = [line for line in lines if all(junk not in line for junk in junk_lines)]

    # Find the last Kai_##### or Claude_##### block
    header_pattern = re.compile(r"^[A-Za-z]+_\d{5}$")
    midi_block = None
    for i in range(len(lines) - 1, 0, -1):  # bottom-up
        if header_pattern.match(lines[i - 1]) and lines[i].startswith("TVRoZA"):
            midi_block = f"{lines[i-1]}\n{lines[i]}"
            break

    if not midi_block:
        logging.warning(f"⚠️ No MIDI64 block found from {agent}")
        return None

    # Save and process as before
    logging.info(f"🎵 Found MIDI64: {lines[i-1]}")
    ...
    # If you want to process MUSE expressions (optional, can be removed if not needed)
    if 'MUSE_' in lines[i-1]:
        try:
            ...
            # If MUSE decode fails, fallback to basic block below
