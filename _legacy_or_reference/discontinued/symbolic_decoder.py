...
class SymbolicDecoder:
    ...
    def parse_expression(self, expression: str) -> Tuple[List[str], List[float], str]:
        ...
        for part in modifier_parts:
            try:
                ...
                raise ValueError(f"Invalid modifier value: {part}")
    ...
    def apply_modifiers(self, cc_values: Dict[int, int], modifiers: List[float]) -> Dict[int, int]:
        ...
        Modifiers map to: [urgency(CC1), brightness(CC74), intimacy(CC11)]
        """
        modifier_mapping = [1, 74, 11]  # CC numbers for urgency, brightness, intimacy
        
        modified_cc = cc_values.copy()
        
        for i, modifier_value in enumerate(modifiers):
            if i < len(modifier_mapping):
                cc_num = modifier_mapping[i]
    ...
    def decode(self, expression: str) -> DecodedExpression:
        ...
        try:
            ...
            modifier_names = ['urgency', 'brightness', 'intimacy']
            modifier_dict = {}
            for i, value in enumerate(modifiers):
                if i < len(modifier_names):
                    modifier_dict[modifier_names[i]] = value
